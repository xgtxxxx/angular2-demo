...
public class OdooService {
    private final Logger LOG = LoggerFactory.getLogger(OdooService.class);
    ...
    public User getCurrentTimeSheetResId(final User user) throws Exception {
        ...
        if (StringUtils.isEmpty(user.getDate()) || user.getDate().equals(DateFormatUtils.format(new Date(),
            "yyyy-MM-dd"))) {
    ...
    private static void initDateToUser(final User user) throws ParseException {
        final GregorianCalendar cal = new GregorianCalendar();
        final Date now = DateUtils.parseDate(user.getDate(), "yyyy-MM-dd");
        cal.setTime(now);
    ...
    public synchronized User getContextDetail(final User user) throws Exception {
        return getContextDetail(user, false);
    }

    public synchronized User getContextDetail(final User user, final boolean force) throws Exception {
        ...
        user.setTimesheetIds(result.getTimesheet_ids() == null || result.getTimesheet_ids().isEmpty()
            ? result.getPeriod_ids()
            : result.getTimesheet_ids());
        user.setDateFrom(result.getDate_from());
        user.setDateTo(result.getDate_to());
        return user;
    }

    public synchronized User getTimeSheetContext(final User user) throws Exception {
    ...
    public synchronized User getProjects(final User user) throws Exception {
        if (user.hasProjects()) {
            return user;
        }
    ...
    private User matchProjectToUser(final User user, final List<List<Object>> result) {
        for (final TimesheetTask task : user.getTimesheetTasks()) {
            ...
            if (!isMatch) {
                LOG.error("Didn't found project '{}' for user {}", task.getProject(), user.getName());
            }
        }
        return user;
    }

    public synchronized User getTasksByProject(final User user) throws Exception {
        if (user.hasTasks()) {
            return user;
        }
    ...
    private void matchTaskToProject(final TimesheetTask task, final List<List<Object>> result) {
        boolean isMatch = false;
        ...
        if (!isMatch) {
            if ("Projects / Core - Web Production Support".equalsIgnoreCase(task.getProjectName())) {
                final String globalTask = "★ Production Support - Maintenance";
                for (final List<Object> t : result) {
                    final String tv = (String) t.get(1);
                    if (globalTask.equalsIgnoreCase(tv)) {
                        task.setTaskId((Integer) t.get(0));
                        task.setTaskName(tv);
                        isMatch = true;
                        break;
    ...
    public User getStageId(final User user) throws Exception {
        ...
        final String url =
            "https://odoo.bridge2solutions.net/web/dataset/call_kw/hr.analytic.timesheet/onchange_task_id";
        for (final TimesheetTask task : user.getTimesheetTasks()) {
            final RequestBody body = new RequestBody();
            final Params params = new Params();
            final Object[] args = new Object[2];
    ...
    public User getAccountId(final User user) throws Exception {
        ...
        final String url =
            "https://odoo.bridge2solutions.net/web/dataset/call_kw/hr.analytic.timesheet/onchange_stage_id";
        for (final TimesheetTask task : user.getTimesheetTasks()) {
            final RequestBody body = new RequestBody();
            final Params params = new Params();
            final Object[] args = new Object[3];
    ...
    private Object createTimesheetTasks(final User user) {
        ...
        for (final TimesheetTask task : user.getTimesheetTasks()) {
            ...
            tm.put("amount", 0);
            String date = task.getCurrentDate();
            if (date == null) {
    ...
    private void findLogsToUser(final User user, final boolean isFull, final OdooResults[] result) {
        ...
        final String today = user.getDate();
        ...
        for (final OdooResults or : result) {
            if (isFull) {
                ...
                if (today.equals(or.getDate())) {
                    logs.addLogBody(new LogBody(
                        or.getUnit_amount(),
                        (String) or.getParent_analytic_id()[1],
                        (String) or.getTask_id()[1]));
                    logs.setLoged(true);
                }
            }
        }
    ...
    private String doRequestPost(
        ...
        if (jsonObject instanceof MultiValueMap) {
            ResponseEntity<String> response = restClient.exchange(
                url,
                HttpMethod.POST,
                createLoginEntity((MultiValueMap) jsonObject),
                String.class);
    ...
    private HttpEntity<String> createEntity(final String sessionId, final String requestBody) throws
        SignatureException {
    ...
    //----------------------end by Allen Wu 2016/10/28--------------------------
    public boolean validateUserHours(User user) {
        boolean flag = true;
        int countHours = 0;
        OdooLog odooLog = user.getLogs();
        if (odooLog != null && odooLog.getBodys().size() >= 5) {
            for (LogBody logBody : odooLog.getBodys()) {
                if (logBody.getHours() < 8) {
                    flag = false;
                }
            }
        } else {
